...
| 5 | rebase_onto_real_source@v1 | 0.9 success; risk was a silently dropped change, not a syntax error | 31/31 assertions; diff vs repo original = **13 hunks, all intended**; comments and entities restored; old-vs-new differs only in comments/entities/whitespace | **LOW on the port itself.** Separate HIGH finding: `Code.gs:49` `AUTH_ENABLED = false` — the live app has **no passcode** while its URL is now public and it collects 身分證號碼 | `src/index.html` promoted back to "deployable candidate"; new open item logged for the auth gate | escalate the auth gate to Dr Chu; still hold deployment at L1 |
